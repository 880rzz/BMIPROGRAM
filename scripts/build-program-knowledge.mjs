...
const enriched=cfg.programs.map(p=>{
 ...
 return {...p,semantic:{rootCauseIds:[...new Set(roots)],rootCauses:[...new Set(roots)].map(id=>semantic.rootCauses[id]),solutionMechanismIds:[...new Set(solutions)],solutionMechanisms:[...new Set(solutions)].map(id=>semantic.solutionMechanisms[id]),ageBands:ageBands(p),teacherProfileUrls:teacherUrls(p.teacher),canonicalScheduleSource:p.url,normalizedRecommendationSource:'https://programvalaszto.magyariskola.at/program-knowledge.json'}};
});
