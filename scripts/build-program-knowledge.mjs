...
const enriched=cfg.programs.map(p=>{
 ...
 return {...p,primaryCategory6:p.primaryCategory6||ctx.window.BMI_PROGRAM_TAXONOMY?.classify(p),semantic:{rootCauseIds:[...new Set(roots)],rootCauses:[...new Set(roots)].map(id=>semantic.rootCauses[id]),solutionMechanismIds:[...new Set(solutions)],solutionMechanisms:[...new Set(solutions)].map(id=>semantic.solutionMechanisms[id]),ageBands:ageBands(p),teacherProfileUrls:teacherUrls(p.teacher),canonicalScheduleSource:p.url,normalizedRecommendationSource:'https://programvalaszto.magyariskola.at/program-knowledge.json'}};
});
const knowledge={schemaVersion:'2026-09-02-v1',schoolYear:cfg.sourcePolicy?.schoolYear||'2026/2027',school:semantic.entity,sourcePriority:{recommendation:'data.js + zenebona-program.js normalized into program-knowledge.json',teacher:'https://tanarok.magyariskola.at/teacher-knowledge.json',registration:'current Wix Event page or explicit partner canonical page'},programCount:enriched.length,programs:enriched};
...
console.log(`OK: ${enriched.length} programs enriched from the same runtime registry used by the recommender`);
