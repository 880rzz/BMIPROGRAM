...
vm.runInNewContext(fs.readFileSync('program-taxonomy.js','utf8'),ctx,{filename:'program-taxonomy.js'});
const cfg=ctx.window.BMI_FINDER;
if(!cfg||!Array.isArray(cfg.programs))throw new Error('BMI_FINDER registry missing');
const taxonomy=ctx.window.BMI_PROGRAM_TAXONOMY;
if(!taxonomy||!taxonomy.fixed||!Array.isArray(taxonomy.categories))throw new Error('BMI_PROGRAM_TAXONOMY missing');
if(cfg.programs.length!==28)throw new Error(`Expected 28 programs, got ${cfg.programs.length}`);
if(!cfg.taxonomyIntegrity?.ok)throw new Error(`Taxonomy integrity failed: ${(cfg.taxonomyIntegrity?.uncategorized||[]).join(', ')}`);
if(Object.keys(taxonomy.fixed).length!==28)throw new Error(`Expected 28 explicit taxonomy mappings, got ${Object.keys(taxonomy.fixed).length}`);
for(const p of cfg.programs){
 const expected=taxonomy.fixed[p.id];
 if(!expected)throw new Error(`No explicit taxonomy mapping for ${p.id}`);
 if(p.primaryCategory6!==expected)throw new Error(`Taxonomy drift for ${p.id}: runtime=${p.primaryCategory6}, expected=${expected}`);
}

const semantic=JSON.parse(fs.readFileSync('semantic-model.json','utf8'));
const profiles={
  'bajka kinga csengele':'bajka-kinga-csengele','horanyi bori':'horanyi-bori','schneider erzsebet':'schneider-erzsebet','makfalvi rita':'makfalvi-rita','fersztl barnabas':'fersztl-barnabas','noszko niki':'noszko-niki','kiss agnes':'kiss-agnes','deakne juhasz eva':'deakne-juhasz-eva','boronkai gabriella':'boronkai-gabriella','korchma zsombor':'korchma-zsombor','hierholcz anna':'hierholcz-anna','pecze adam':'pecze-adam','trencsenyi klara':'trencsenyi-klara','falusi dora':'falusi-dora','hupczik andrea':'hupczik-andrea','telenko eva':'telenko-eva','varga bernadette':'varga-bernadette','veres tamas':'veres-tamas','banhalmi norbert':'banhalmi-norbert','balogh david':'balogh-david','sipos tibor':'sipos-tibor','orban dalma':'orban-dalma','egri monika':'egri-monika','dapin hajnalka judit':'dapin-hajnalka-judit','pohl balazs':'pohl-balazs','poser piroska ildiko':'poser-piroska-ildiko'
};
const norm=s=>String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,' ').trim();
function teacherUrls(raw){const n=norm(raw),out=[];for(const [alias,slug] of Object.entries(profiles))if(n.includes(alias))out.push(`https://tanarok.magyariskola.at/#${slug}`);return[...new Set(out)]}
...
const enriched=cfg.programs.map(p=>{
 ...
 if(!roots.length)roots.push('diaspora-social-fragmentation');
 if(!solutions.length)solutions.push('peer-community');
 return {...p,primaryCategory6:taxonomy.fixed[p.id],semantic:{rootCauseIds:[...new Set(roots)],rootCauses:[...new Set(roots)].map(id=>semantic.rootCauses[id]),solutionMechanismIds:[...new Set(solutions)],solutionMechanisms:[...new Set(solutions)].map(id=>semantic.solutionMechanisms[id]),ageBands:ageBands(p),teacherProfileUrls:teacherUrls(p.teacher),canonicalScheduleSource:p.url,normalizedRecommendationSource:'https://programvalaszto.magyariskola.at/program-knowledge.json'}};
});
const knowledge={schemaVersion:'2026-09-02-v2',schoolYear:cfg.sourcePolicy?.schoolYear||'2026/2027',school:semantic.entity,categories6:taxonomy.categories,sourcePriority:{recommendation:'data.js + zenebona-program.js + explicit program-taxonomy.js normalized into program-knowledge.json',teacher:'https://tanarok.magyariskola.at/teacher-knowledge.json',registration:'current Wix Event page or explicit partner canonical page'},programCount:enriched.length,programs:enriched};
for(const p of enriched){if(!p.painPoint||!p.outcome)throw new Error(`Missing painPoint/outcome: ${p.id}`);if(!p.semantic.rootCauseIds.length)throw new Error(`Missing root cause: ${p.id}`);if(!p.semantic.solutionMechanismIds.length)throw new Error(`Missing solution mechanism: ${p.id}`);if(!(p.minAge>=0&&p.maxAge>=p.minAge))throw new Error(`Invalid age range: ${p.id}`);if(!p.when||!p.location||!p.teacher)throw new Error(`Missing schedule/location/teacher: ${p.id}`);if(p.primaryCategory6!==taxonomy.fixed[p.id])throw new Error(`Generated taxonomy drift: ${p.id}`)}
